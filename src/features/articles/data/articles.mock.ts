...
export interface Article {
  ...
  authors: string[]
  year: number
  type: "artigo científico" | "revisão" | "caso clínico" | "estudo epidemiológico" | "outro"
  journal?: string
}
...
// Helper function to get occurrences filtered by search
export function filterOccurrencesBySearch(occurrences: Occurrence[], query: string): Occurrence[] {
  if (!query.trim()) return occurrences
  const lowerQuery = query.toLowerCase()
  return occurrences.filter(
    (occ) =>
      occ.city.toLowerCase().includes(lowerQuery) ||
      occ.state.toLowerCase().includes(lowerQuery)
  )
}
...
// Helper function to get host category color
